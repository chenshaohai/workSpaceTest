...
#pragma mark - 注册
- (void)reginster:(NSString *)scannedResult
{
    __weak typeof(self) weakSelf = self;
    [[ASingleton shareInstance]startLoadingInView:self.view];
    [IWHttpTool getWithURL:scannedResult params:nil success:^(id json) {
        [[ASingleton shareInstance]stopLoadingView];
        IWLog(@"json=======%@",json);
        if (json == nil || ![json isKindOfClass:[NSDictionary class]]) {
            [weakSelf changeFail:@"无法识别"];
            return ;
        }
        ...
        // 扫描成功回调注册界面
        if ([_inToType isEqualToString:@"0"]) {
            
        }else{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"SGScanningQRCode" object:nil userInfo:json[@"data"]];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        if ([obj.stringValue hasPrefix:@"http"]) {
            ...
            [self reginster:obj.stringValue];
...
//            [self.navigationController pushViewController:jumpVC animated:YES];
            // 首页
            if ([_inToType isEqualToString:@"0"]) {
                IWNearCommitBillVC *vc = [[IWNearCommitBillVC alloc] initWithName:nil andShoppingId:obj.stringValue];
                [self.navigationController pushViewController:vc animated:YES];
            }else{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"SGScanningQRCode" object:nil userInfo:@{@"key":obj.stringValue}];
                [self.navigationController popViewControllerAnimated:YES];
            }
